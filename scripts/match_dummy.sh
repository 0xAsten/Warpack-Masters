...
export FIGHT_STSTEM_ADDRESS=$(cat ./manifest_dev.json | jq -r '.contracts[] | select(.tag == "Warpacks-fight_system").address')

echo "---------------------------------------------------------------------------"
echo world : $WORLD_ADDRESS
echo " "
echo fight system : $FIGHT_STSTEM_ADDRESS
echo "---------------------------------------------------------------------------"

sozo execute --world $WORLD_ADDRESS $FIGHT_STSTEM_ADDRESS match_dummy --wait --rpc-url $STARKNET_RPC_URL \
